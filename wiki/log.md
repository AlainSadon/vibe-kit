...
| 2026-06-29 | dec-quality-security-hooks, cap-drift-detectie, drift-check.mjs, run-checks, start-project, README, AGENTS.md | Quality- en security-hooks toegevoegd als opt-in command-hooks (default null); agent detecteert + stelt voor zoals het testcommando. Architectuur-hook bewust uit scope. |
| 2026-06-29 | dec-positionering, README | Marktpositionering vastgelegd na landschapsonderzoek (SDD-frameworks als Spec Kit/BMAD/Kiro, memory banks als Cline Memory Bank); niche = drift-handhaving + leanheid. |
| 2026-06-29 | dec-config-bestand, drift-check.mjs, vibe-kit.config.mjs, add-to-project.mjs, cap-drift-detectie, skills, docs | Instellingen verhuisd naar `vibe-kit.config.mjs` (motor/inhoud-scheiding) zodat een upgrade ze niet overschrijft; script merget config over interne defaults. |
| 2026-06-29 | dec-bestaand-project-installatie, add-to-project.mjs, README, CHANGELOG | `--upgrade`-modus toegevoegd: ververst alleen de motor (drift-check + skills), laat inhoud/config met rust, meldt AGENTS.md + CI alleen bij verschil. End-to-end getest op een nagebootst project. |
| 2026-06-29 | skill upgrade-kit, AGENTS.md, dec-bestaand-project-installatie, CHANGELOG | Skill `upgrade-kit` toegevoegd + in de skills-tabel: maakt de upgrade-route agent-aanroepbaar (assistent kent het commando zonder de README). |
